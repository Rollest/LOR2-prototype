...
func _ready():
	...
	unitBody = get_node("Sprite2D/CharacterBody2D")
	print(unitBody)
...
func _listener_selected(id,type):
	print(id,"  ",unitBody.get_instance_id(), "  ", type)
	if(id == unitBody.get_instance_id()):
		if(type == "motion"): 
			_mouse_on()
		elif(type == "press"):
			_mouse_on_pressed()
	elif(id != unitBody.get_instance_id()):
		if(type == "motion"): 
			_mouse_off()
		elif(type == "press"):
			_mouse_on_unpressed()
