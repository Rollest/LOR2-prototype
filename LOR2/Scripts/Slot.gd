...
func _ready():
	...
	if(target && target.get_node("Slot")):
		arcs.dict_slot_A_B[self] = [global_position, target.get_node("Slot").global_position]
...
func change_target(newTarget:Unit):
	target = newTarget
	arcs.dict_slot_A_B[self] = [global_position, target.get_node("EnemySlot").global_position]
	pass
